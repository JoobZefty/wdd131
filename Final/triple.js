function getCards(number){
    ...
    for (let i = 0; i < number; i=i+3){
        ...
        htmlList.push(`<div class="card" id="${(i+2)}" value="${value1}"></div>`);
    }
    return htmlList;
}
...
function flipCard(cardId, flipIdList, correctList, number, health) {
    ...
    //match checker
    if(flipIdList.length === 3){
        if ((document.getElementById(flipIdList[0]).getAttribute('value') === document.getElementById(flipIdList[1]).getAttribute('value')) && (document.getElementById(flipIdList[0]).getAttribute('value') === document.getElementById(flipIdList[2]).getAttribute('value'))){
            for (i=0;i<flipIdList.length;i++){
                    console.log("match!! >:)");
        ...
        cards.forEach(card => {
            card.addEventListener('click', () => {
                ...
                if (correctList.length === number){
                    ...
                    number += 3;
                    healthMultiplier +=1;
                    health = (Math.floor(healthMultiplier * 4.4));
