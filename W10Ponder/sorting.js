...
let query = 'a'; // in prove use inputs .value
...
let filterHikes = hikes.filter(function(item){
    return (
        item.name.toLowerCase().includes(hikeQuery.toLowerCase()) ||
        item.description.toLowerCase().includes(hikeQuery.toLowerCase()) ||
        item.tags.find(tag => tag.toLowerCase().includes(hikeQuery.toLowerCase()))
    );
});
